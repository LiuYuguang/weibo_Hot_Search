...
def crawl():
	url = 'https://i.snssdk.com/feoffline/hot_list/template/hot_list/'
	headers = {}
...
def dump(data):
	...
	path = f'{base_path}/{day}/toutiao_{time}.html'
...
db = sqlite3.connect(base_path + "/toutiao.db")
def create_db():
	cursor = db.cursor()
	cursor.execute(f'CREATE TABLE IF NOT EXISTS TOUTIAO_HOT (UPDATETIME INTEGER NOT NULL, RANK INTEGER NOT NULL, TOPIC TEXT, COUNT INTEGER, ATTACH TEXT, PRIMARY KEY(UPDATETIME,RANK));')
	db.commit()
	cursor.close()
	pass
...
def insert_db(data_list):
	...
	for updatetime,rank,topic,count,attach in data_list:
		cursor.execute(f'INSERT INTO TOUTIAO_HOT (UPDATETIME, RANK, TOPIC, COUNT, ATTACH) VALUES (?,?,?,?,?);',(updatetime,rank,topic,count,attach))
	db.commit()
	cursor.close()
	pass
